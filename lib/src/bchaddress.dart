...
class BCHAddress {
  ...
  BCHAddress.fromHex(String hexPubKey, NetworkType networkType) {
  ...
  ///
  /// Also see [NetworkType]
  BCHAddress.fromPublicKey(BCHPublicKey pubKey, NetworkType networkType) {
  ...
  ///
  /// Also see [NetworkType]
  BCHAddress.fromCompressedPubKey(
      List<int> pubkeyBytes, NetworkType networkType) {
  ...
  ///
  BCHAddress.fromBase58(String base58Address) {
  ...
  String toBase58() {
    ...
    var rawHash =
        HEX.decode(_publicKeyHash).map((elem) => elem.toSigned(8)).toList();
  ...
  ///
  /// This is only really needed because BCH has two different test networks
