import 'package:bitbox/src/bchaddress.dart';
import 'package:bitbox/src/networks.dart';
...
//        _publicKey = ECPublicKey(_point, _domainParams);
  ...
  /// public key.
  BCHAddress toAddress(NetworkType nat) {
    //generate compressed addresses by default
    List<int> buffer = _point.getEncoded(_point.isCompressed);

    if (_point.isCompressed) {
      return BCHAddress.fromCompressedPubKey(buffer, nat);
    } else {
      return BCHAddress.fromHex(HEX.encode(buffer), nat);
    }
  }
