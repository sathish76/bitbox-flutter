...
//        _publicKey = ECPublicKey(_point, _domainParams);
  ...
  BCHPublicKey.fromDER(List<int> buffer, {bool strict = true}) {
    if (buffer.isEmpty) {
      throw Exception('Empty compressed DER buffer');
    }
...
//        _publicKey = ECPublicKey(_point, _domainParams);
  ...
  BCHPublicKey.fromHex(String pubkey, {bool strict = true}) {
    if (pubkey.trim() == '') {
      throw Exception('Empty compressed public key string');
    }
  ...
  ECPoint _transformDER(List<int> buf, bool strict) {
    ...
    if (buf[0] == 0x04 || (!strict && (buf[0] == 0x06 || buf[0] == 0x07))) {
      ...
      if (xbuf.length != 32 || ybuf.length != 32 || buf.length != 65) {
        throw Exception('Length of x and y must be 32 bytes');
      }
      ...
      throw Exception('Invalid DER format public key');
    }
    return point;
  }
